...
pub struct Implementation<'x, T> {
    map: Arena<Node<T>, &'x TheAlloc>,
    first: Option<Index>,
    last: Option<Index>,
}

impl<'x, T> Implementation<'x, T> {
    fn allocate(&mut self, value: T) -> Index {
...
impl<'x, T> DoubleLinkedList<'x, T> for Implementation<'x, T> {
    ...
    fn new(alloc: &'x TheAlloc, capacity: usize) -> Self {
        Self {
            map: Arena::with_capacity_in(capacity, alloc),
            first: None,
            last: None,
        }
    }
    ...
    fn insert_after(&mut self, node: Self::NodeRef, value: T) -> Self::NodeRef {
        let next = self
            ...
            .get(node)
            .map(|n| n.next)
            .expect("insert_after called with node not in list");
    ...
    fn insert_before(&mut self, node: Self::NodeRef, value: T) -> Self::NodeRef {
        let prec = self
            ...
            .get(node)
            .map(|n| n.prec)
            .expect("insert_before called with node not in list");
        let new_key = self.allocate(value);
