...
impl<K: Key, V, A: Allocator> SlotMap<K, V, A> {
    ...
    /// let hello = messages.insert("Hello");
    /// ```
    pub fn with_capacity_and_key_in(capacity: usize, alloc: A) -> Self {
        // Create slots with a sentinel at index 0.
        // We don't actually use the sentinel for anything currently, but
        // HopSlotMap does, and if we want keys to remain valid through
        // conversion we have to have one as well.
        let mut slots = Vec::with_capacity_in(capacity + 1, alloc);
        slots.push(Slot {
            u: SlotUnion { next_free: 0 },
            version: 0,
        });
...
impl<K: Key, V, A: Allocator + Clone> Clone for SlotMap<K, V, A>
where
    V: Clone,
{
    fn clone(&self) -> Self {
...
#[derive(Debug)]
pub struct Drain<'a, K: 'a + Key, V: 'a, A: Allocator> {
...
#[derive(Debug, Clone)]
pub struct IntoIter<K: Key, V, A: Allocator + Clone> {
    num_left: usize,
    slots: Enumerate<alloc::vec::IntoIter<Slot<V>, A>>,
    _k: PhantomData<fn(K) -> K>,
}
...
impl<K: Key, V, A: Allocator + Clone> Iterator for IntoIter<K, V, A> {
    type Item = (K, V);
...
impl<K: Key, V, A: Allocator + Clone> IntoIterator for SlotMap<K, V, A> {
    type Item = (K, V);
...
impl<K: Key, V, A: Allocator + Clone> FusedIterator for IntoIter<K, V, A> {}
...
impl<K: Key, V, A: Allocator + Clone> ExactSizeIterator for IntoIter<K, V, A> {}
